            ...
            scrape_date = _.min(_.map(settings.keys('projects'),
                function(project_key) {
                    ...
                    return !fs.existsSync(scrape_file) ? moment.invalid() :
                        moment(fs.readFileSync(scrape_file), "YYYY-MM-DD HH:mm:ss");
                }
            ));
        }
    }
...
module.exports = function(app) {
    app.get('/', function(req, res) {
        ...
        var status_time = 'time';
        if (!scrape_date.isValid()) {
            status_time = 'time-never';
        }
        else if (moment().diff(scrape_date, 'days') > 1) {
            status_time = 'time-old';
        }
