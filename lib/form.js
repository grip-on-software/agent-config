...
fields.string = function(opts) {
    opts.attrs = opts.attrs || {
        ...
        placeholder: opts.placeholder ? render(opts.placeholder) : null
    };
    ...
    f.parse = function(raw_data) {
        if (raw_data === config.existing_value) {
    ...
    f.setOptionValue = function(optionData, config_form, settings, credentials) {
        ...
        try {
            if (optionData.length <= 2) {
                this.value = getSectionValue(section, configuration);
            }
            else {
                option = optionData[2].split('.', 2);
                this.value = configuration.get(section, (option.length > 1 ?
...
function getCredentialsSection(config_form, credentials) {
...
function create_forms(body, settings, credentials) {
    ...
    function hasForm(formName, sequence) {
        if (body && formName + '_' + sequence in body) {
            ...
            const section = credentials.sections()[sequence-1];
