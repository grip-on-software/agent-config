...
fields.string = function(opts) {
    ...
    const f = fields.plain_string(opts);
    const bind = f.bind,
        classes = f.classes,
        toHTML = f.toHTML;
...
fields.map = function(opts) {
    ...
    const toHTML = f.toHTML,
          bind = f.bind;
    f.items = [f.fields];
    f.bind = function(data) {
        f.setData(data);
        const fields = f.fields;
        const bound = _.reduce(f.items, function(accumulator, item, index) {
            f.fields = item;
            const b = bind(_.mapValues(data, function(fieldData) {
...
function buildForm(form, formName, validator, className='component') {
    ...
    Object.keys(formData.fields).forEach(function(fieldName) {
        ...
        const field = fields[data.field](data);
...
function create_forms(body, settings, credentials) {
    ...
    Object.keys(form).forEach(function(formName) {
        let formData = buildForm(form, formName, validator);
