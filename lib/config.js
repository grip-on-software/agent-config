...
function createSources(sources, vcs_type, domain, group, checkouts) {
    checkouts = typeof checkouts === 'undefined' ? [] :
        checkouts.filter(function(source) {
            return source.value !== '';
        });
...
function remove_original_credentials(credentials, config_forms, index, key) {
    const domain = config_forms[1+index].form.fields[key].original_value;
    credentials.removeSection(domain);
}
...
${projects[0].value === '' ? 'skip_dropin=0' : ''}`;
    ...
    [].concat(data.version_control_domain).forEach(function(domain, index) {
        remove_original_credentials(credentials, config_forms, index,
