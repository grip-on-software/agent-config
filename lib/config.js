...
function createKey(auth, domain, key) {
    let keyFile = path.join(options.key_path, 'id_rsa');
...
function set_credentials(credentials, data, index, sources) {
    ...
    const keyFile = createKey(auth, domain, key);
    createSources(sources, vcsType, domain, group, checkouts);

    let credentialsEnv = '-';
    let credentialsAssign = '';
    if (auth !== 'user_pass') {
...
function writeSettings(data, projects) {
    ...
    replaceSection(settings, 'sonar');
    if (data.sonar_url) {
        setOption(settings, 'sonar', 'name', 'sonar-data');
        setOption(settings, 'sonar', 'url', data.sonar_url);
    }
...
${projects[0].value === '' ? 'skip_dropin=0' : ''}`;
    ...
    if (sources.length > 0 && projects.reduce(function(result, project) {
            return result && project.value === '';
        }, true) && exports.has_value(data.jira_key)
    ) {
...
exports.update = function update_config(data, configForms) {
    const projects = typeof data.quality_report_name === "undefined" ?
