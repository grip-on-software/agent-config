...
const version_control_options = {
    ...
    gitlab_group_name: function(config_form, settings, credentials) {
        const section = getVersionControlSection(config_form, credentials);
...
function update_config(data, config_forms) {
    function set_credentials(main_key, index) {
        const domain = [].concat(data.version_control_domain)[index],
              ...
              group = [].concat(data.gitlab_group_name)[index],
        ...
        if (auth === 'deploy_key' && token !== '' && token !== '<existing>') {
            fs.writeFileSync(path.join(key_path, 'id_rsa'), token);
        }
        if (data.quality_report_name === '' && config_has_value(main_key) &&
            vcs_type === 'gitlab' && index == 0
        ) {
            fs.writeFileSync(path.join(export_path, main_key, 'data_sources.json'),
                JSON.stringify([{
                    "type": vcs_type,
                    "url": `http://${data.version_control_domain}/${group}/${group}.git`,
                    "name": "dummy repository"
                }])
            );
        }

        return (auth === 'user_pass' ? '' : `${credentials_env}=id_rsa`);
    }
    ...
    async.reduce(config_forms, {"success": [], "other": []}, 
        function(states, config_form, callback) {
            ...
            if (!formStates.other.length) {
                ...
                res.write(`<pre>${util.inspect(data)}</pre>`);
