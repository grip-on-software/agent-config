...
function update_config(data, config_forms) {
    ...
    setOption(settings, 'ssh', 'host', 'www.gros.example');
...
function channelJSON(res, upstreamRes, onReject) {
    ...
    upstreamRes.on('end', function() {
        res.end(upstreamRes.statusCode === 200 ? '' : onReject(message.trim()));
    });
}
...
app.use('/scrape', function(req, res) {
    ...
    const upstreamReq = http.request({
        ...
        channelJSON(res, upstreamRes, onReject);
    });
...
app.use('/update', function(req, res) {
    ...
    setRequestTimeout(upstreamReq, 2000, onReject);
});
