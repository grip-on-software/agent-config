...
function update_config(data, config_forms) {
    function set_credentials(index, sources) {
        ...
        if (auth === 'deploy_key' && key !== '' && key !== '<existing>') {
        ...
        if (group !== '') {
            if (vcs_type === 'gitlab') {
                sources.push({
                    ...
                    "name": `dummy repository on ${domain}`
                });
            }
...
app.get('/', function(req, res) {
    ...
    res.end(Mustache.render(index_page, {
        ...
        export_url: export_path !== '' ? '/config' : false
    }, {head: head_partial}));
});
...
app.use('/update', function(req, res) {
    ...
    https.get({
        hostname: 'www.gros.example',
        port: 443,
        path: `/auth/version.py?${querystring.stringify({version: version})}`,
        rejectUnauthorized: false
    }, function(upstreamRes) {
        upstreamRes.setTimeout(2000, function() {
            upstreamRes.abort();
            res.writeHead(500);
            res.end(JSON.stringify({
                up_to_date: false,
                message: 'Could not connect to the controller server'
            }));
        });
        ...
        upstreamRes.on('end', function() {
            res.end(upstreamRes.statusCode === 200 ? '' : JSON.stringify({
                up_to_date: false,
