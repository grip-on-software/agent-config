...
var string = function(opts) {
    opts.attrs = opts.attrs || {required: opts.required};
...
function update_config(data) {
    ...
    const readConfig = function(config, filename) {
    ...
    const settings = new configparser();
    readConfig(settings, 'settings.cfg');

    replaceSection(settings, 'bigboat');
    settings.set('bigboat', 'host', data.bigboat_url);
    settings.set('bigboat', 'key', data.bigboat_key);

    var subproject_keys = data.jira_key.split(' ');
    const main_key = subproject_keys.shift();
    replaceSection(settings, 'projects');
    settings.set('projects', main_key, data.quality_report_name);

    replaceSection(settings, 'subprojects');
    subproject_keys.forEach(function(subproject_key) {
        settings.set('subprojects', subproject_key, main_key);
    });

    replaceSection(settings, 'jenkins');
    settings.set('jenkins', 'host', data.jenkins_host);
    settings.set('jenkins', 'username', data.jenkins_user);
    settings.set('jenkins', 'password', data.jenkins_token);

    writeConfig(settings, 'settings.cfg');

    const credentials = new configparser();
    readConfig(credentials, 'credentials.cfg');
    ...
    credentials.set(domain, 'env',
        (auth == 'user_pass' ? '-' : 'SSH_IDENITITY')
    );
    credentials.set(domain, 'username',
        (auth == 'user_pass' ? data.version_control_user : '-')
    );
    credentials.set(domain, 'password',
        (auth == 'user_pass' ? data.version_control_token : '-')
    );
    credentials.set(domain, 'github_api_url', '-');
    credentials.set(domain, 'github_token',
        (vcs_type == 'github' ? data.version_control_token : '-')
    );
    credentials.set(domain, 'github_bots', '-');
    credentials.set(domain, 'tfs', (vcs_type == 'tfs' ? 'true' : '-'));
    credentials.set(domain, 'group',
