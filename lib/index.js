...
const export_path = getConfig('EXPORT_PATH', 'export_path');
const config_path = getConfig('CONFIG_PATH', 'config_path');
const key_path = getConfig('IDENTITY_PATH', 'key_path');
const listen_address = getConfig('LISTEN_ADDR', 'listen_address');
const listen_port = Number(getConfig('LISTEN_PORT', 'listen_port'));
const ssh_host = getConfig('SSH_HOST', 'ssh_host');
const agent_host = getConfig('AGENT_HOST', 'agent_host');
const agent_port = Number(getConfig('AGENT_PORT', 'agent_port'));
const visualization_url = getConfig('VISUALIZATION_URL', 'visualization_url');
        ...
        function(states, config_form, callback) {
            ...
            if (!formStates.other.length) {
                ...
                res.end(Mustache.render(success_page, {
                    ...
                    scrape_url: config_has_value(agent_host) ? '/scrape' : false,
                }, {head: head_partial}));
            }
            else {
                res.writeHead(200, { 'Content-Type': 'text/html' });
...
if (app.get('env') === 'dev-sync') {
    ...
    bs.init({ logSnippet: false, reloadOnRestart: false }, bs.reload);
...
app.get('/', function(req, res) {
    ...
    var config_ok = fs.existsSync(path.join(config_path, 'settings.cfg'));
    var scrape_date = null;
    if (config_ok) {
        const settings = read_config('settings.cfg');
        if (!settings.hasSection('projects') ||
            settings.keys('projects').length === 0 ||
            !config_has_value(settings.keys('projects')[0])
        ) {
            config_ok = false;
        }
        else {
    ...
    res.end(Mustache.render(index_page, {
        ...
        visualization_url: config_has_value(visualization_url) ? visualization_url : false
    }, {head: head_partial}));
});
...
app.post('/scrape', function(req, res) {
    ...
    if (!config_has_value(agent_host)) {
        res.writeHead(500);
...
const url = 'http://' + listen_address + ':' + listen_port + '/';
